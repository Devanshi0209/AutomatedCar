...
def carla_preprocess(image):
	...
	image=image/255
	return image
def carla_preprocess_bigimage(image):
...
def store_image_data(imagedata,ourvehicle,model,model2,gameDisplay,clock,category_index):
	...
	throttlevalue=1.0-(current_speed/5.0)
	copy_i3=np.copy(i3)
	...
	i5=cv2.resize(i4,(512,512))
	i6=i5[219:475,256:512,:]
	detections=run_inference_for_single_image(model2,i6)
	#image_np_with_detections = i6.copy()

	outlist=[category_index.get(value) for index,value in enumerate(detections['detection_classes']) if detections['detection_scores'][index] > 0.9]
	if len(outlist)>=1:
		print(outlist[0])
	...
	carImg=pygame.surfarray.make_surface(i5.swapaxes(0,1))
	carImg = pygame.transform.smoothscale(carImg, (512,512)) 
...
def store_image_data1(imagedata,ourvehicle):

	i=np.array(imagedata.raw_data)
	i2=i.reshape((IMAGEHEIGHT,IMAGEWIDTH,4))
	i3=i2[:, :, : 3]
	directory=r'D:\WindowsNoEditor\PythonAPI\examples\dataset-traffic-vehicles'
	os.chdir(directory)
	cv2.imwrite('out%08d.jpg' % imagedata.frame,i3)

	'''
	i4=i3[219:475,256:512,:]
	directory=r'D:\WindowsNoEditor\PythonAPI\examples\dataset-traffic-vehicles'
	os.chdir(directory)

	cv2.imwrite('out%08d.jpg' % imagedata.frame,i4)
...
if __name__=="__main__":
	...
	try:
		...
		#ourvehicle.set_autopilot(True)


		model=load_model('D:\WindowsNoEditor\PythonAPI\examples\save_model-rgbdata-nvdia-1output-balanced\saved_model')
		model2=tf.saved_model.load("D:/WindowsNoEditor/PythonAPI/examples/workspace/exported-models/my_ssd_model/saved_model")
